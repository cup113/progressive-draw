  ...
  <main class="flex flex-col gap-4 items-center">
    ...
    <section>
      <TransitionGroup tag="ol" class="flex flex-wrap justify-center gap-2" name="fade">
        <li v-for="entrant in entrants.winners" :key="entrant.name" class="flex items-center gap-1 bg-lime-300 rounded-md p-4">
          <div>{{ entrant.name }}</div>
        </li>
      </TransitionGroup>
    </section>
    <section>
      <div class="text-center" v-if="entrants.handle">{{ entrants.nextWinSec.toFixed(1) }} 秒后抽取下一人</div>
      <TransitionGroup tag="ol" class="flex flex-col gap-2 w-[40rem] mx-auto" name="fade">
        <li v-for="entrant in entrants.rankedEntrants" :key="entrant.name" :ref="storeRef"
          ...
          <div>{{ entrant.distance.toFixed(0) }}</div>
        </li>
      </TransitionGroup>
    </section>
  </main>
